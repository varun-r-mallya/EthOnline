...
contract DecentralizedRideHailing {
    ...
    event RideCreated(uint256 indexed rideId, address indexed driver, address indexed rider, Location startLocation, Location endLocation, uint256 fare);
    ...
    modifier onlyResponder() {
        ...
        require(msg.sender != address(0), "Only a valid responder can perform this action");
        _;
    }
    ...
    function createRide(
        ...
        emit RideCreated(rideCounter, driver, msg.sender, startLocation, endLocation, fare);
    }
    ...
    function completeEmergency(uint256 rideId) external onlyRider(rideId) {
        Emergency storage emergency = emergencies[rideId];
        require(emergency.isAccepted, "Emergency must be accepted before completion");
